...
DDIM [^2]使用另一个方法推导 $$q\left(\mathbf{x}_{t-1} \mid \mathbf{x}_{t}, \mathbf{x}_{0}\right)$$ 
...
为什么 $$\sqrt{1 - {\alpha}_{t-1}}\cdot\mathbf{\epsilon_{t-1}} $$ 可以写成 $$\sqrt{1 - {\alpha}_{t-1} - \sigma_t^2} \cdot\mathbf{\epsilon_t} + \sigma_t\mathbf{\epsilon}$$ ？
